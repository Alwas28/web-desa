...
@section('page-sub', 'Master data kategori berita dan konten')
  ...
  <div class="lg:col-span-2 space-y-4">
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-start gap-3 flex-1 min-w-0">
            ...
            <div class="flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center mt-0.5"
                 ...
                 class="{{ !$kat->warna ? 'bg-slate-100 dark:bg-slate-800' : '' }}">
              <i class="ti ti-tag" style="{{ $kat->warna ? 'color:' . $kat->warna : '' }}"></i>
            </div>
            <div class="min-w-0">
              ...
              <div class="flex items-center gap-1.5 mt-1.5 text-xs text-slate-400">
                <i class="ti ti-article text-sm"></i>
                <span>Digunakan di fitur Berita</span>
              </div>
            </div>
          </div>

          <div class="flex items-center gap-2 flex-shrink-0">
            @if(Auth::user()->hasPermission('edit.kategori'))
            <button type="button"
            ...
            @endif
            @if(Auth::user()->hasPermission('hapus.kategori'))
            <form method="POST" action="{{ route('admin.master.kategori.destroy', $kat) }}" class="m-0">
              @csrf @method('DELETE')
              <button type="button" onclick="konfirmasiHapus(this.closest('form'), 'Kategori \'{{ addslashes($kat->nama) }}\' akan dihapus permanen.', 'Hapus Kategori')"
    ...
    @empty
      <div class="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-12 text-center">
        <i class="ti ti-tag-off text-3xl text-slate-300 dark:text-slate-600 block mb-2"></i>
        <p class="text-sm text-slate-400">Belum ada kategori. Buat kategori pertama di panel kanan.</p>
      </div>
    @endforelse
  </div>

  {{-- ── Kanan: Form ───────────────────────────────── --}}
  @if(Auth::user()->hasPermission('tambah.kategori') || Auth::user()->hasPermission('edit.kategori'))
  <div class="space-y-4">
    <h2 class="font-semibold text-base" id="formTitle">Tambah Kategori</h2>

    <div class="rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5">
      <form method="POST" id="kategoriForm" action="{{ route('admin.master.kategori.store') }}" class="space-y-4">
        @csrf
        ...
        <div>
          ...
          <input type="text" name="nama" id="f_nama" required
            ...
            value="{{ old('nama') }}" placeholder="Contoh: Berita Desa">
        ...
        <div>
          ...
          <div class="flex items-center rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden focus-within:ring-2 focus-within:ring-brand-500">
            ...
            <input type="text" name="slug" id="f_slug"
              ...
              value="{{ old('slug') }}" placeholder="berita-desa" pattern="[a-z0-9-]+">
          </div>
        ...
        <div>
          <label class="block text-sm font-medium mb-1.5">Warna Label</label>
          ...
          <p class="text-xs text-slate-400 mt-1">Opsional. Digunakan sebagai label berwarna pada tampilan publik.</p>
        </div>
        ...
        <div>
          ...
          <textarea name="deskripsi" id="f_deskripsi" rows="3"
            ...
            placeholder="Penjelasan singkat tentang kategori ini…">{{ old('deskripsi') }}</textarea>
        </div>
...
@section('scripts')
<script>
const STORE_URL  = '{{ route('admin.master.kategori.store') }}';
const UPDATE_BASE = '{{ url('admin/master/kategori') }}/';
...
/* ── Sinkronisasi color picker ↔ text hex ── */
...
function clearWarna() {
  ...
  hexInput.value = '';
  // set name blank so server receives empty
  colorInput.name = '';
  hexInput.name = 'warna';
}
...
function openEdit(id, nama, slug, warna, deskripsi) {
  ...
  const sf = document.getElementById('f_slug');
  sf.value = slug; sf.dataset.manual = '1';
  const cv = warna || '#059669';
  colorInput.value = cv;
  colorInput.name  = '';
  hexInput.value   = warna || '';
  hexInput.name    = 'warna';
  document.getElementById('f_deskripsi').value = deskripsi;
  document.getElementById('metodField');
...
function resetForm() {
  ...
  colorInput.name = ''; hexInput.name = 'warna';
  colorInput.value = '#059669'; hexInput.value = '';
